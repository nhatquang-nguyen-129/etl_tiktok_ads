...
- `main.py` retrieves `secret_advertiser_id` from `{COMPANY}_secret_{DEPARTMENT}_tiktok_account_id_{ACCOUNT}`

- `main.py` retrieves `secret_advertiser_name` from `projects/{PROJECT}/secrets/{secret_advertiser_id}/versions/latest`

- `main.py` retrieves `secret_token_id` from `{COMPANY}_secret_all_tiktok_token_access_user`
